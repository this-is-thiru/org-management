...
public class UserService {
    ...
    public ClientPersonnelDetail getClientPersonnelDetailByUserId(String userId) {
        Optional<ClientPersonnelDetail> userDetailOptional = clientPersonnelDetailsRepository.findByUserId(userId);
        if (userDetailOptional.isPresent()) {
            return userDetailOptional.get();
        }
        throw new IllegalArgumentException("Employee with userId " + userId + " not found");
    }
